	...
	if ( '' != $search_form_template ) {
		...
		if ( 'html5' == $format ) {
			$form = '<form role="search" method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<label>
					<input type="search" class="search-field" placeholder="' . esc_attr_x( 'Search&hellip;', 'placeholder' ) . '" value="' . get_search_query() . '" name="s" title="' . esc_attr_x( 'bob:', 'label' ) . '" />
				</label>
			...
			$form = '<form role="search" method="get" id="searchform" class="searchform" action="' . esc_url( home_url( '/' ) ) . '">
				<div>
					...
					<input type="submit" id="searchsubmit" value="'. esc_attr_x( 'Go', 'submit button' ) .'" />
				</div>
			</form>';
		}
	}
...
function wp_get_archives($args = '') {
	...
	if ( 'monthly' == $type ) {
	...
	} elseif ('monthly_show_posts' == $type) {
		$curr_month = "";
		...
		if ( $results ) {
			foreach ( (array) $results as $result ) {
				$post_month = date('F', strtotime($result->post_date));
				$post_year = date('Y', strtotime($result->post_date));
				if($curr_month == "" || $curr_month != $post_month) {
					// $output .= $post_month . " " . $post_year;
					$url = get_month_link( $post_year, $post_month );
					$text = sprintf(__('%1$s %2$d'), $post_month, $post_year);
					$output .= get_archives_link($url, $text, $format, "<li class='h2'>", "</li>");
				}
